...
# Encontrar inversa de la matriz por metodo gaussjordan
def eliminacionInversa(matriz_a_reducir, log_func=print):
    filas = len(matriz_a_reducir)
    columnas = len(matriz_a_reducir[0])
    ...
    for i in range(filas):
        matriz_a_reducir[i] += identidad[i]

    # Aplicar eliminacion de Gauss-Jordan
    eliminacionGaussJordan(matriz_a_reducir, log_func=log_func)
